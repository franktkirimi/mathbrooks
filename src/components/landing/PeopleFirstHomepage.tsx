  ...
  <>
    <section className="relative isolate min-h-[min(760px,100svh)] overflow-hidden bg-background px-5 py-28 text-foreground sm:px-6 md:py-36 lg:py-0">
      <div className="relative mx-auto flex min-h-[calc(min(760px,100svh)-14rem)] max-w-7xl items-center lg:min-h-screen">
        <AnimatedSection className="max-w-3xl">
          <h1 className="font-display text-[clamp(3.15rem,9vw,7.5rem)] font-bold leading-[0.92] tracking-[-0.065em] text-foreground">Technology built around <span className="text-primary">people.</span></h1>
          <p className="mt-6 max-w-xl text-lg font-normal leading-7 text-muted-foreground sm:text-xl sm:leading-9 md:mt-8 md:text-2xl">MathBrooks builds intelligent technology for the real world.</p>
    ...
    <section id="what-we-build" className="px-5 py-20 sm:px-6 md:py-28 lg:py-36">
      <div className="mx-auto grid max-w-6xl gap-10 lg:gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
        <AnimatedSection>
          <h2 className="font-display text-4xl font-bold leading-[0.98] tracking-[-0.055em] text-foreground sm:text-5xl md:text-7xl">Built to solve the problems holding you back.</h2>
          <div className="mt-7 max-w-xl text-base font-normal leading-8 text-muted-foreground md:text-lg"><p>If your organisation has a real-world challenge or a project that needs to work better, we begin by understanding what is at stake.</p></div>
    ...
    <section id="industries" className="relative isolate overflow-hidden px-6 py-28 md:py-36"><LazyIndustryOrbit /><div className="relative mx-auto max-w-6xl"><AnimatedSection className="max-w-3xl"><h2 className="font-display text-4xl font-bold leading-[1.02] tracking-[-0.045em] text-foreground md:text-6xl">Technology for the real world.</h2></AnimatedSection><div className="mx-auto mt-12 grid max-w-4xl grid-cols-2 justify-items-center gap-x-5 gap-y-8 sm:grid-cols-3">{industries.map((industry, index) => <AnimatedSection key={industry.name} delay={index * 50}>{industry.href ? <a href={industry.href} className="group flex size-32 items-center justify-center rounded-full border border-border-strong bg-card p-5 text-center shadow-card transition duration-300 hover:-translate-y-1 hover:border-primary hover:shadow-overlay focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-4 sm:size-36 md:size-40" aria-label={`View ${industry.name} prototype`}><span className="font-body text-base font-semibold leading-tight text-foreground transition-colors group-hover:text-primary sm:text-lg">{industry.name}</span></a> : <button type="button" onClick={() => toast({ title: `${industry.name} prototype`, description: "This prototype is coming soon." })} className="group flex size-32 items-center justify-center rounded-full border border-border-strong bg-card p-5 text-center shadow-card transition duration-300 hover:-translate-y-1 hover:border-primary hover:shadow-overlay focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-4 sm:size-36 md:size-40" aria-label={`${industry.name} prototype coming soon`}><span className="font-body text-base font-semibold leading-tight text-foreground transition-colors group-hover:text-primary sm:text-lg">{industry.name}</span></button>}</AnimatedSection>)}</div></div></section>

    <section id="technology" className="px-6 py-28 md:py-36"><div className="mx-auto max-w-6xl"><AnimatedSection className="max-w-3xl"><h2 className="font-display text-4xl font-bold leading-[1.02] tracking-[-0.045em] text-foreground md:text-6xl">The technology behind the work.</h2><p className="mt-5 text-lg font-normal leading-8 text-muted-foreground">We combine these tools according to the problem being solved—not because any one of them is the point.</p></AnimatedSection><div className="mt-12 grid gap-px overflow-hidden rounded-2xl border border-border bg-border sm:grid-cols-2 lg:grid-cols-3">{technologyTools.map((tool, index) => { const Icon = capabilityIcons[index]; return <div key={tool} className="bg-card p-7"><Icon className="h-6 w-6 text-[hsl(var(--teal))]" /><h3 className="mt-8 font-display text-2xl font-bold tracking-[-0.02em] text-foreground">{tool}</h3><p className="mt-3 text-sm font-normal leading-7 text-muted-foreground">A practical part of the system, selected because it can help people do more.</p></div>; })}</div></div></section>

    <section className="bg-foreground px-6 py-28 text-white md:py-36"><div className="mx-auto max-w-6xl"><AnimatedSection><h2 className="max-w-3xl font-display text-4xl font-bold leading-[1.02] tracking-[-0.045em] md:text-6xl">One ecosystem. Built from the real world out.</h2><div className="mt-12 grid gap-4 md:grid-cols-5">{["People & real-world problems", "MathBrooks", "AI · software · computing · connected technology", "Products", "Real-world applications"].map((item, index) => <div key={item} className="border-t border-white/30 pt-4"><span className="text-xs text-white/50">0{index + 1}</span><p className="mt-3 text-sm leading-6">{item}</p></div>)}</div><div className="mt-10 flex flex-wrap gap-4"><Button asChild><Link to="/about">Our approach <ArrowRight /></Link></Button><Link to="/contact" className="inline-flex items-center gap-2 px-5 py-3 text-sm text-white hover:underline">Talk to MathBrooks <ArrowRight className="h-4 w-4" /></Link></div></AnimatedSection></div></section>
  </>
);
