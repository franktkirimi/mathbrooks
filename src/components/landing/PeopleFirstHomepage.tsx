...
import MineLandingScene from "@/components/landing/MineLandingScene";
...
const PeopleFirstHomepage = () => (
  <>
    <section className="relative isolate min-h-[min(760px,100svh)] overflow-hidden bg-[#090a09] px-5 py-28 text-white sm:px-6 md:py-36 lg:py-0">
      <MineLandingScene />
      <div aria-hidden="true" className="absolute inset-0 bg-[linear-gradient(90deg,rgba(4,5,5,0.92)_0%,rgba(4,5,5,0.79)_34%,rgba(4,5,5,0.26)_63%,rgba(4,5,5,0.15)_100%)]" />
      <div aria-hidden="true" className="absolute inset-0 bg-[linear-gradient(0deg,rgba(4,5,5,0.76)_0%,transparent_32%,transparent_72%,rgba(4,5,5,0.28)_100%)]" />
      <div className="relative mx-auto flex min-h-[calc(min(760px,100svh)-14rem)] max-w-7xl items-center lg:min-h-screen">
        <AnimatedSection className="max-w-3xl">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#f5a050] sm:text-sm">MathBrooks Things · Research</p>
          <h1 className="mt-5 font-body text-[clamp(3.15rem,9vw,7.5rem)] font-semibold leading-[0.92] tracking-[-0.065em] text-white">Technology built around <span className="text-[#f5a050]">people.</span></h1>
          <p className="mt-6 max-w-xl text-lg font-light leading-7 text-white/75 sm:text-xl sm:leading-9 md:mt-8 md:text-2xl">MathBrooks builds intelligent technology for the real world.</p>
          <div className="mt-8 flex flex-col gap-3 sm:mt-10 sm:flex-row sm:flex-wrap sm:gap-4"><Button asChild size="lg" className="w-full bg-white text-[#101313] hover:bg-white/90 sm:w-auto"><a href="#what-we-build">Explore what we build <ArrowRight /></a></Button><Button asChild variant="outline" size="lg" className="w-full border-white/35 bg-white/5 text-white hover:bg-white/15 hover:text-white sm:w-auto"><a href="#technology">Our technology</a></Button></div>
        </AnimatedSection>
      </div>
      <p className="absolute bottom-7 left-5 text-[0.65rem] font-medium uppercase tracking-[0.16em] text-white/50 sm:left-6 lg:left-[max(1.5rem,calc((100%-80rem)/2))]">Soko A01 · Mine operations study</p>
    </section>
    ...
    <section id="what-we-build" className="px-5 py-20 sm:px-6 md:py-28 lg:py-36">
      <div className="mx-auto grid max-w-6xl gap-10 lg:gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
        <AnimatedSection>
          <h2 className="font-body text-4xl font-semibold leading-[0.98] tracking-[-0.055em] text-foreground sm:text-5xl md:text-7xl">Built to solve the problems holding you back.</h2>
